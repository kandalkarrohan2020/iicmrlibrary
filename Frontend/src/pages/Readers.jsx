...
const Readers = () => {
  ...
  const status = async (id) => {
    if (!window.confirm("Are you sure to change this Reader status?")) return;
  ...
  const payFine = async (id) => {
    if (!window.confirm("Are you sure to Receive Payment From this Reader ?"))
      return;

    try {
      const response = await fetch(
        URI + `/admin/readers/update/fine/payment/${id}`,
        {
          method: "PUT",
          credentials: "include", //  Ensures cookies are sent
          headers: {
      ...
      console.error("Error Pay Fine :", error);
    }
  };
  ...
  const fetchMenus = async () => {
    try {
      const response = await fetch(URI + "/admin/readers/get/menus", {
        method: "GET",
        credentials: "include", // Ensures cookies are sent
        headers: {
          "Content-Type": "application/json",
  ...
  const assignTask = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(
        URI + `/admin/readers/assign/tasks/${readerId}`,
        {
          method: "PUT",
          credentials: "include",
          headers: {
  ...
  const filteredData = readers?.filter((item) => {
    ...
    const getReaderStatus = () => {
      ...
      if (item.paymentstatus === "Unpaid") return "Unpaid";
      return "";
    };

    const matchesReader =
      !readerPaymentStatus || getReaderStatus() === readerPaymentStatus;
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-full md:w-[500px] lg:w-[700px] max-h-[70vh] overflow-scroll scrollbar-hide bg-white py-8 pb-16 px-4 sm:px-6 border border-[#cfcfcf33] rounded-tl-lg rounded-tr-lg md:rounded-lg">
          ...
          <form onSubmit={add}>
            <div className="grid gap-6 md:gap-4 grid-cols-1 md:grid-cols-2 ">
              ...
              <div className="w-full">
                ...
                <select
                  required
                  className="w-full mt-[4px] text-[16px] font-medium p-2 border border-[#00000033] rounded-[4px] focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none bg-transparent"
      ...
      <div
        ...
        } z-[61] property-form overflow-scroll scrollbar-hide w-full fixed bottom-0 md:bottom-auto`}
      >
        <div className="w-full md:w-[500px] lg:w-[700px] max-h-[70vh] overflow-scroll scrollbar-hide bg-white py-8 pb-16 px-4 sm:px-6 border border-[#cfcfcf33] rounded-tl-lg rounded-tr-lg md:rounded-lg">
          <div className="flex items-center justify-between mb-4">
